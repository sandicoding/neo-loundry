	...
	class ModelTransaksi extends CI_Model
	{
		...
		public function showdata()
		{
			return $this->db->query("
				SELECT id_transaksi,jml_kilo,tgl_transaksi,tgl_ambil,keterangan, total,(tb_kasir.nama) AS kasir,(tb_konsumen.nama) AS konsumen,(tb_paket.nama) AS paket
				FROM tb_transaksi
		...
		{
			return $this->db->query("
				SELECT id_transaksi,jml_kilo,tgl_transaksi,tgl_ambil,keterangan, total,(tb_kasir.nama) AS kasir,(tb_konsumen.nama) AS konsumen,(tb_paket.nama) AS paket,tb_konsumen.id_konsumen,harga
				FROM tb_transaksi 
				INNER JOIN tb_kasir ON tb_transaksi.id_kasir=tb_kasir.id_kasir
				INNER JOIN tb_konsumen ON tb_transaksi.id_konsumen=tb_konsumen.id_konsumen
				INNER JOIN tb_paket ON tb_transaksi.id_paket=tb_paket.id_paket
		...
		{
			$sql = $this->db->query("SELECT id_transaksi,(tb_kasir.nama) as kasir, (tb_konsumen.nama) as konsumen, (tb_paket.nama) as paket, tgl_transaksi,keterangan,jml_kilo,total
				FROM tb_transaksi
				INNER JOIN tb_kasir ON tb_transaksi.id_kasir=tb_kasir.id_kasir
				INNER JOIN tb_konsumen ON tb_transaksi.id_konsumen=tb_konsumen.id_konsumen
				INNER JOIN tb_paket ON tb_transaksi.id_paket=tb_paket.id_paket
